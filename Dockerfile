...
RUN apt-get install -y pkg-config build-essential bison swig2.0 python-dev python-pip psmisc gstreamer1.0 gstreamer1.0-dev gstreamer1.0-tools python-gst-1.0 libxml2-dev libxslt-dev autoconf automake libtool ffmpeg
RUN pip install pycaption requests
ADD install-pocketsphinx.sh /tmp/
RUN sh /tmp/install-pocketsphinx.sh
RUN useradd -d /home/user -s /bin/bash -m user
ADD caption.py adapt-from-captions.py run.sh /home/user/
RUN chmod +x /home/user/caption.py /home/user/adapt-from-captions.py /home/user/run.sh
USER user
